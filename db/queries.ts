...
export const getUnits = cache(async () => {
    ...
    const normalizeData = data.map((unit) => {
        const lessonsWithCompletedStauts = unit.lessons.map((lesson) => {
        ...
        return { ...unit, lessons: lessonsWithCompletedStauts };
    });
...
export const getLessonPercentage = cache(async () => {
    ...
    const completedChanllenges = lesson.challenges
        .filter((challenge) => challenge.completed);

    const percentage = Math.round(
        (completedChanllenges.length / lesson.challenges.length) * 100,
    );
